...
with open(input_csv, mode='r', encoding='utf-8-sig') as file:
    ...
    for row in reader:
        ...
        commands.append(f"sh cdp neighbors {interface}")
        commands.append(f"! This should return the neighbor connected to {interface}, which is {leaf}")

# Define the output file path
output_file = os.path.join(current_dir, 'pre_commands.txt')
